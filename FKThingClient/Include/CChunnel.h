...
//------------------------------------------------------------------------
class CChunnel : public IChunnel
{
public:
	// 释放
	...
	*/
	...
	// 取得传送方式
	virtual DWORD				GetTransMode(void);

	// 取得目标点信息
	virtual SCHUNNEL_TARGET *	GetTargetInfor(DWORD &dwCount);
...
private:
	UID							m_uid;
	// 属性表
	SCHUNNEL_PROPERTY_LIST		m_Property;
	// 地图物件
