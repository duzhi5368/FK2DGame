...
public:
	...
	*/
	...
	// 取得帐号的DB ID
	virtual DWORD			GetUserDBID(void);
	
	// 取得角色的DB ID
	virtual DWORD			GetActorDBID(void);

	// 取得角色的氏族ID
	virtual DWORD			GetActorClanID(void);
...
protected:
	UID						m_uid;
	// 属性列表
	SACTOR_PROPERTY_PRIVATE	m_Property;
	// 逻辑对像分类
	CThingClass				m_ThingClass;
	// 旧头像资源ID
	int						m_dwOldHeadResID;
	// 是否处在死亡态
	BOOL					m_bIsDying;
